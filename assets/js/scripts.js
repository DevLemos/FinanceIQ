...
async function formValidationLogin() {
  document.getElementById('formLoginId').addEventListener("submit", async function(event){
    ...
    .catch(erro =>{
      apresentarMensagem("Erro!","Algo deu errado na solicitação.","error", function () { });
...
async function formValidationCreateAccount() {
  document.getElementById("formCreateAccountId").addEventListener("submit", async function(event) {
      ...
      .then(data =>{
        if(data.success){
          apresentarMensagem("Cadastro realizado!",data.message,"success", function () { document.getElementById("tabAccountAccess").click();});
        }
        else
          apresentarMensagem("Erro!",data.message,"error", function () {  document.getElementById("tabAccountAccess").click();});
      })
      .catch(erro =>{
        apresentarMensagem("Erro!","Algo deu errado na solicitação.","error", function () { window.location.href = 'index.html'});
        console.error('Error',erro);
      })
